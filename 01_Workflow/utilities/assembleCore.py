...
class PDB:
    def __init__(self, target, center, lig_num_atoms=60):
        ...
        with open(f'{self.target}', 'r') as f:
    ...
    def determine_threshold(self, target_atoms=None, target_residues=None, target_protein_atoms=None, verbose=False):
        assert np.sum([target_atoms is not None, target_residues is not None, target_protein_atoms is not None]) == 1, "Of the three options (target_atoms, target_residues, target_protein_atoms), only one can be true"
        
        self.included_atoms = 0
        self.protein_atoms = 0
        self.included_residues = 0
        self.thres = 5.0
        if type(target_atoms) == int: # Use target atoms
            while (self.included_atoms < target_atoms):
                ...
                self.included_atoms, self.protein_atoms, self.included_residues = self.join_segments(verbose=False)
            # In case included atoms is larger than 928, revert one step
            if self.included_atoms > 925:
                self.thres -= 0.01
                self.determine_close_residues(thres=self.thres, verbose=False)
                self.included_atoms, self.protein_atoms, self.included_residues = self.join_segments(verbose=False)
        elif type(target_residues) == int: # Use fixed number of residues
            ...
            if self.included_atoms > 925:
                ...
                self.determine_close_residues(thres=self.thres, verbose=False)
                self.included_atoms, self.protein_atoms, self.included_residues = self.join_segments(verbose=False)
        elif type(target_protein_atoms) == int: # Use fixed number of protein atoms
            while (self.protein_atoms < target_protein_atoms):
                if round(self.thres * 100) % 100 == 0:
                    print(f'{self.thres:.2f} A ...')
                self.thres += 0.01
                self.determine_close_residues(thres=self.thres, verbose=False)
    ...
    def join_segments(self, verbose=True):
        ...
        self.est_protein_atoms = self.est_receptor_atoms + self.est_cap_atoms
        self.est_included_atoms = self.est_protein_atoms + self.est_lig_atoms
        self.est_num_residues = len(self.cont_res) + 2 * len(self.segments) + 1
        if verbose:
            self.print_joined_segments()

        return self.est_included_atoms, self.est_protein_atoms, self.est_num_residues
    ...
    def write_all_PDBs(self, dry_run=True, path='./', write_script=False, overwrite_coor_file=None):
        # override_coordinates is a flex result from flexible docking
        try:
            os.makedirs(path)
        except:
            pass
        ...
        for ii in range(len(self.segments)):
            ...
            self.write_PDB(ii, dry_run = dry_run, path = path, first = first_flag, last = last_flag, 
                           overwrite_coor_file = overwrite_coor_file)
        self.write_PDB('all', dry_run = dry_run, path = path, overwrite_coor_file = overwrite_coor_file)
        # pass
        if write_script:
            self.write_all_scripts(path)

    def write_PDB(self, seg, dry_run=True, path='./', first=False, last=False, overwrite_coor_file=None):

        if overwrite_coor_file is not None:
            with open(overwrite_coor_file, 'r') as f:
                cont = f.readlines()
            overwrite_coor = parse_flex_PDBQT(cont)
        else:
            overwrite_coor = None
        #print(overwrite_coor)
        ...
        if dry_run == True:
            for res, atom in zip(self.target_resid, self.target_pdb):
                if res in seg_list:
                    if atom[1] != '0' and atom[2][0] != 'H':
                        this_atom = overwritten(atom, overwrite_coor)
                        print(atom_line(res, this_atom, counter),end='')
                        counter += 1
        else: # Write to file
            ...
            with open(f'{path}/{file_name}','w') as f:
                for res, atom in zip(self.target_resid, self.target_pdb):
                    if res in seg_list:
                        if atom[1] != '0' and atom[2][0] != 'H':
                            this_atom = overwritten(atom, overwrite_coor)
                            f.write(atom_line(res, this_atom, counter))
...
def overwritten(atom, overwrite_coor=None):
    if overwrite_coor is None:
        return atom
    ret_atom = atom
    for a in overwrite_coor:
        if (a[2] == ret_atom[2]) and (a[3] == ret_atom[3]) and (a[4] == ret_atom[4]) and (a[5] == str(ret_atom[12])):
            #print(f'Overwriting the coordinates for {atom}')
            ret_atom[6] = a[6]
            ret_atom[7] = a[7]
            ret_atom[8] = a[8]
    return ret_atom
...
def parse_flex_PDBQT(PDBQT_content):
    splitPDB = []
    for line in PDBQT_content:
        if 'ATOM' in line:
            splitPDB.append([line[:6].strip(),    # 0. 'ATOM'
                             line[6:11].strip(),  # 1. index
                             line[13:17].strip(), # 2. atom name - slightly different from PDB
                             line[17:20].strip(), # 3. residue name - slightly different from PDB
                             line[21],            # 4. chain name
                             line[22:27].strip(), # 5. residue number
                             line[30:38].strip(), # 6. X
                             line[38:46].strip(), # 7. Y
