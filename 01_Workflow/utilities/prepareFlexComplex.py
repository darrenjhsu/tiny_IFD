...
ps_dir = sys.argv[1] + '/assembly/'
flex_dir = sys.argv[1] + '/flex_assembly'
...
for ii in lig_names:
    try:
        ...
        lig_index = lig_id.split('_')[-1]     # Test1_1
        #lig_prmtop_fname = lig_base + lig_param_suffix + '.prmtop'
        #print(f'{lig_prmtop_fname}')
        try:
            ps0 = parmed.load_file(f'{flex_dir}_{lig_index}/apo_continue_fix_CA.prmtop',f'{flex_dir}_{lig_index}/apo_continue.inpcrd')
        except:
            try:
                ps0 = parmed.load_file(f'{flex_dir}_{lig_index}/apo_continue.prmtop',f'{flex_dir}_{lig_index}/apo_continue.inpcrd')
            except:
                print(f"{flex_dir}_{lig_index}/apo_continue files not found! Skipping ...")
                continue

        ligand_structure = parmed.load_file(f'{lig_param_dir}/{lig_prmtop_fname}',
                                            f'{lig_dir}/{ii}')
        complex_structure = ps0 + ligand_structure
        print('Complex assembled by prepareFlexComplex')
...
lig_base = lig_names[0].split('_')[0]   # 
